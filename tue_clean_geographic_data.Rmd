...
```{r}
dat <- read_tsv("C:/Users/az64mycy/Dropbox (iDiv)/teaching/2021_big_data_biogeography_physalis/example_data/day2_data_quality_and_sampling_bias/raw_occurrences_malvaceae_sa.csv", guess_max = 25000, quote = "")
...
check <- flags %>% select(contains(".")) %>% 
  mutate(.summary_marine = ifelse(all(), "gut", "schlecht"))

%>% 
  # since flags also contains a summary column for all test, we need to fix this as well. 
  mutate(.summary = ifelse(all(. %>% select(contains("\."))), "gut", "schlecht") )
