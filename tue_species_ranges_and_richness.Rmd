...
# Input data needed
- the .csv file with cleaned species occurrences from the geographic cleaning exercise
...
4. Visualize species richness in an equal area raster (`raster`, `spTransform`, `coordinates`).
5. Write the result rasters to a file on your disk
...
```{r}
dat <- read_csv("example_data/day2_data_quality_and_sampling_bias/cleaned_occurrences_malvaceae.csv")
...
ggsave("plot_occurrence_numbers.pdf")
...
# Calculate ranges
ranges <- dat %>% 
  #filter(decimalLongitude < 60) %>% #focussing on America and Africa
  CalcRange(terrestrial = TRUE)
...
## 4. Visualise species richness in an equal area raster
...
## Output generated
- A plot of occurrence numbers
- A shape file with species ranges based on convex hulls
- Plots of an equal area raster of species richness
